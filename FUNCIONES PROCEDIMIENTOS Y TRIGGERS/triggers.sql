...
CREATE DEFINER=`root`@`localhost` TRIGGER `nuevo_cliente` AFTER INSERT ON `cliente` FOR EACH ROW begin 
	insert into matricula(cod_matricula, tipo, fecha, CLIENTE_id_cliente)
	values (uuid(), 'completo', curdate(),new.id_cliente);
end;

CREATE DEFINER=`root`@`localhost` TRIGGER `nuevo_telefono` AFTER UPDATE ON `entrenadores` FOR EACH ROW begin 
	if new.num_telefono<> old.num_telefono then 
		insert into notificaciones (mensaje, fecha, ENTRENADORES_id_entrenadores)values 
		('Su número de telefono ha sido actualizado', curdate(), new.id_entrenador);
	end if; 
end;
